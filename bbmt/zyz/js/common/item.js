...
var SwiperTpl=[
    ...
    '<div class="item-content">',
       ' <div class="item-content-top">',
            '<p class="dec"><span class="sign-price-title">单价 </span> <span class="sign-price-num">&yen;{{price}} </span><span class="market-price-title">市场价 </span><span class="market-price-num">&yen; {{market_price}}</span></span></p>',
...
var tplhideTpl=[
                '<ul class="d_ul2" data-itemid="{{item_id}}" id="itemid">',
					...
					'<li>{{title}}</li>',
					'<li><span class="base-color">&yen;<span id="totlePrice">{{if specs.length!=0}}{{specs[0].price}}{{else}}{{price}}{{/if}}</span></span><span class="color1 marLeft-10">(单价&yen<span id="price">{{if specs.length!=0}}{{specs[0].price1}}{{else}} {{market_price}}  {{/if}}</span>)</span></li>',
				'</ul>',
				'<div class="d_fenlei">',
                    '{{if specs.length!=0}}',
                        '<ul>',
...
var Item=function(data){
    ...
    Item.prototype.init=function(){
        ...
        $('.d_fl').find('li').eq(2).append(html3);
        ...
        $('#d_jia').click(function() {
            if($('.active_').size()>0){
                ...
                if(num<totle){
                    ...
                    alert('最多只能购买这么多', "forbidden");
                }
            }else{
                alert('请选择规格', "forbidden");
            }
        });
